...
- **It must be inside the gateway's `allowed_dirs`** or the submit is refused outright — `cwd … is not under any allowed_dirs`. `ab info` lists them, and `/tmp` usually is **not** among them, so use `/tmp/ab-<task>` only when it actually is and prefer `$HOME` or the cluster's scratch filesystem otherwise. The cwd check is the narrow one: the *upload store* does live under `$TMPDIR` by default and `ab upload`/`ab download` reach it regardless, but that exception is for the store alone — a scratch cwd you invent under `/tmp` is neither allowed as a cwd nor fetchable afterwards.
